...
#include "RichShieldIRremoteInt.h"
...
void IRsend::enableIROut(int khz) {
 ...
 TIMSK0 &= ~_BV(TOIE0); //Timer2 Overflow Interrupt
  pinMode(5, OUTPUT);
  digitalWrite(5, LOW); // When not sending PWM, we want it low
  
  // WGM02:00 = 101: phase-correct PWM with OCRA as top
  ...
  // COM2A = 10: disconnect OC2A
  //OCR2A = 200;
  //OCR2B = 200;
  // COM2B = 10: disconnect OC2B; to send signal set to 10: OC2B non-inverted
  // WGM2 = 101: phase-correct PWM with OCRA as top
  // CS2 = 10: //no prescaling
  // The top value for the timer.  The modulation frequency will be SYSCLOCK / 2 / OCR2A.
...
/**Only for NEC IR remote control***/
int IRrecv::decode() {
...
int IRrecv::isReleased()
{
  if(flag_repeat)return 0;//not release the key yet
  else return 1;
}
...
long IRrecv::decodeNEC(decode_results *results) {
  ...
  /***************************************************************************/
 /*Modified by OPEN-SMART Team, because the NEC protocol send LSB first but not MSB first*/
  ...
  results->value = data;
  results->decode_type = NEC;
  results->usercode = buf[0];
  results->keycode = buf[2];
  usercode = buf[0];
  keycode = buf[2];
  flag_repeat = 0;
  return DECODED;
