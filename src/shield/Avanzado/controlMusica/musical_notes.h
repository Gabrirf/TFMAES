...
void playSong(long[] notes, long[] times){
  for(int i=0; i<notes.legnth(); i++){
    playTone(notes[i], times[i]);
  }
}

void playSong(long[] notes, long[] times, int control){
  for(int i=0; i<notes.legnth(); i++){
    long value = analogRead(A0)/100;
    playTone(notes[i], times[i]*value);
  }
}
